...
#
# # Display setting for console
# pd.set_option("display.max_rows", 200)          # print max 200 rows
# pd.set_option("display.max_columns", None)      # print all columns
# pd.set_option("display.width", 0)               # auto line break
# pd.set_option("display.max_colwidth", None)     # no shorten cell
#
# # read file
...
# # print(df.columns)
#
# # Task1: Data normalization and cleaning
# # ===== Task 1.1 Salary =====
...
# df["min_salary"], df["max_salary"], df["salary_unit"], df["salary_note"] = zip(*parsed)
#
# # Quick check
# print(df[["salary","min_salary","max_salary","salary_unit","salary_note"]]
...
#
# # Apply to dataframe
# df["city"], df["district"] = zip(*df["address"].apply(lambda s: split_address_joined(s, max_pairs=None, dedupe=True)))
# df["city_district_pairs_str"] = df["address"].apply(format_pairs)
#
# # Quick check
# print(df[["address","city","district","city_district_pairs_str"]].head(10).to_string(index=False))
#
# # ===== Task 1.3  job_title =====
#
...
#     return "other_it"
#
# # Apply detailed group + seniority
# df["job_title_group"] = df["job_title"].apply(_job_group)
# df["job_seniority"]   = df["job_title"].apply(_job_seniority)
#
...
#     "other_it":                  "other",
# }
# df["job_title_group_big"] = df["job_title_group"].map(collapse_map).fillna("other")
#
# # --- Quick checks ---
# print(df[["job_title", "job_title_group", "job_title_group_big", "job_seniority"]]
#       .head(12).to_string(index=False))
#
# # Sanity: confirm infra/platform roles don't fall into 'other_it'
# # _mask_infra = df["job_title"].astype(str).str.contains(r"(?i)\binfra|infrastructure|platform\b", regex=True, na=False)
# # print("\n# Sample — rows with 'infra/infrastructure/platform'")
# # print(df.loc[_mask_infra, ["job_title", "job_title_group", "job_title_group_big", "job_seniority"]]
# #         .head(10).to_string(index=False))
#
# # Optional distributions
# big_counts = Counter(df["job_title_group_big"].fillna("other"))
# print("\n# job_title_group_big distribution")
# for k, v in big_counts.most_common():
#     print(f"{k:18} {v}")
#
